...
- Canonical full role name: `full`; unset/empty role env → `full`; value `all` aliases to `full`
- Integrate tool boolean argument remains named `all` (not the role)
- Docs: permissions only — no “prefer full/split” marketing copy; stating unset→`full` as fact is OK
...
| `internal/mcp/server.go` | Role → register + instructions |
...
Expected: PASS for role tests (server/tools may still compile; if tools.go still references old names, fix only in later tasks — role_test should pass)
...
In `registerSharedTools`: only status, abort, cleanup — delete `mergesiding_list` tool and `handleList`.
...
Update `handleAbort` — stop reading cleanup booleans; call `abort.Abort(paths.Default(), slug)` once Task 3 changes the signature. **For this task only**, if Abort still has 4 params, pass `false, false` temporarily OR do Task 3 first in the same session before compiling — prefer completing Task 3 abort signature before finishing this step’s compile.
...
Expected: PASS (may fail until Task 3 if Abort signature already changed — order Task 3 next if compile errors)
...
**Interfaces:**
- Produces: `func Abort(p paths.Paths, taskSlug string) (*models.TaskRecord, error)` — no cleanup params; does not call `cleanup.CleanupTask`

- [ ] **Step 1: Write failing abort tests**

`internal/abort/abort_test.go` — use a temp MERGESIDING_HOME / store pattern from existing `store` or `start` tests if available. Minimal approach:
...
Inspect `internal/store/store_test.go` and `internal/paths/paths.go` for how tests set home. Mirror that: create an `active` task JSON, call `Abort`, assert status `aborted`, assert worktree paths untouched (no need to create real git if Abort no longer calls cleanup — just assert no error and status).
...
- [ ] **Step 2: Run test — expect fail if signature still has cleanup or cleanup still invoked**
...
- [ ] **Step 4: CLI — reject old flags; drop list**
...
Update `printHelp` — remove `list` line; abort line without cleanup flags.
...
Expected: exit 2 and the stderr message (slug may also fail load — flag check should run first)
...
- full: all of the above; note unset/empty `MERGESIDING_MCP_ROLE` → full; `all` accepted as alias of `full`

Remove integrate_all / list mentions. State abort does not clean disk; cleanup after done/aborted.

Config examples may show writer+scheduler and/or a single server without role env / with `full` — without “recommended” wording.
...
Snippet: short pointer to Skill + `start → ready → integrate` + one line on MCP role tool access → `docs/mcp.md`.
...
Expected: no hits that recommend removed surface or preferred-role marketing (factual “unset defaults to full” is OK).
...
- Abort signature change coordinated between Tasks 2–3 (implement Abort API before MCP compile finishes)
